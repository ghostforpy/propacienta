...
@method_decorator(name="list", decorator=swagger_auto_schema(tags=["diseases"]))
class DiseasesViewSet(ListModelMixin, GenericViewSet):
    ...
    def get_queryset(self):
        ...
        if pacient_id != 0:
            disease_type = queryparams.get(
                "diseaseType", None
            )  # chronic or transferred
            if disease_type in ("chronic", "transferred"):
...
class BaseAuthRetrieveDischargeEpicrisFileAndImageView(RetrieveAPIView):
    ...
    def get_object(self):
        ...
        qs = self.queryset.filter(discharge_epicris__pacient__id=pacient_id).filter(
...
class AuthRetrieveDischargeEpicrisImageView(
    BaseAuthRetrieveDischargeEpicrisFileAndImageView
):
    """
...
class AuthRetrieveDischargeEpicrisFilesView(
    BaseAuthRetrieveDischargeEpicrisFileAndImageView
):
    """
...
@method_decorator(
    name="post", decorator=swagger_auto_schema(tags=["transferred-diseases"])
)
@method_decorator(
    name="get", decorator=swagger_auto_schema(tags=["transferred-diseases"])
)
...
@method_decorator(
    name="delete", decorator=swagger_auto_schema(tags=["transferred-diseases"])
)
...
@method_decorator(name="post", decorator=swagger_auto_schema(tags=["chronic-diseases"]))
@method_decorator(name="get", decorator=swagger_auto_schema(tags=["chronic-diseases"]))
class DischargeEpicrisCreateListView(CreateAPIView, ListAPIView):
    ...
    def get_queryset(self):
        ...
        return (
            super()
            .get_queryset()
            .filter(pacient__id=pacient_id, chronic_disease__disease__id=disease_id)
        )


@method_decorator(
    name="delete", decorator=swagger_auto_schema(tags=["chronic-diseases"])
)
