...
from hospitals.models import get_default_hospital, get_default_hospital_id
...
class AppointmentOrderSerializer(serializers.ModelSerializer):
    hospital = serializers.PrimaryKeyRelatedField(
        read_only=True,
        default=get_default_hospital_id()
    )
    ...
    def validate(self, data):
        ...
        try:
            ...
            w = WorkDay.objects.filter(
                doctor=doctor,
                hospital=get_default_hospital_id(),  # default hospital
                date=dt.date(),
