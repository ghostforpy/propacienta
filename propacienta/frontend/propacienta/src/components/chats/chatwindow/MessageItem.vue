<template>
  <div :id="message.id" class="sc-message">
    <div
      ...
      :class="{
        sent: message.sender === $store.getters.id,
        received:
          message.sender !== $store.getters.id && message.type !== 'system',
...
export default {
  ...
  computed: {
    ...
    chatImageUrl() {
      if (this.user.doctor_id != null) {
        return this.user.doctor_foto != null
          ? this.user.doctor_foto
          : require("@/assets/default_doctor_avatar.png");
      } else {
        return require("@/assets/default-pacient.jpg");
      }

      // return (this.user && this.user.doctor_foto) || chatIcon;
    },
    messageColors() {
